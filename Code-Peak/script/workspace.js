...
function renderCourses(courses) {
  ...
  courses.forEach((course) => {
    ...
    // Обработка формы
    card.querySelector(`#form-${course.courseID}`).addEventListener("submit", function (e) {
      e.preventDefault();


       const formData = new FormData();
        formData.append("title", document.getElementById(`title-${course.courseID}`).value.trim());
        formData.append("description", document.getElementById(`desc-${course.courseID}`).value.trim());
        formData.append("cost", document.getElementById(`cost-${course.courseID}`).value);
        formData.append("time", document.getElementById(`time-${course.courseID}`).value);

        const fileInput = document.getElementById(`banner-course-edit-${course.courseID}`);5
...
function showEditForm(id) {
  ...
  document.querySelectorAll(".container-edit").forEach(v => v.style.display = "flex");
...
function loadUserCourses() {
  ...
  .then(res => res.json())
  .then(courses => 
    renderUserCourses(courses.courses))
...
function renderUserCourses(courses) {
  ...
  if (courses.length === 0) {
    container.innerHTML = "<p class='donthavecoursesuser'>Вы ещё не записались ни на один курс.</p>";
    return;
  }
...
function renderLessons(lessons) {
  ...
  function updateStats() {
    const completedCount = lessons.filter(lesson => completedLessons.has(lesson.lessonsID)).length;
  ...
  lessons.forEach((lesson, index) => {
    ...
    lessonDiv.innerHTML = `
      <div class="lesson-title">
        ...
        <input type="checkbox" ${isCompleted ? 'checked' : ''} class="lesson-checkbox">
      </div>
      <div class="lesson-content">
          <iframe width="100%" height="300" src="${lesson.linkVideo}" frameBorder="0" allow="clipboard-write; autoplay" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>
        <p>${lesson.descriptionLessons}</p>
      </div>
    `;
    ...
    lessonDiv.querySelector('.lesson-title').addEventListener('click', (e) => {
      ...
      currentContent.style.display = currentContent.style.display === 'none' ? 'block' : 'none';
    });
