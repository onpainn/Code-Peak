      ...
      <div class="courses_block_item">
        <div class="courses_block_item_img" style="background-image: url('http://82.202.137.140:5000${
          ...
          <div class="courses_block_item_pay">
            ...
            <button onclick="enrollCourse(${course.courseID})">Записаться</button>
          </div>
        </div>
...
function enrollCourse(courseId) {
  ...
  .then(res => {
    if (!res.ok) throw new Error(showMessageUser("Вы не можете дважды записаться на курс"));
    return res.json();
  })
  .then(() => {
    showMessageUser("Вы успешно записались на курс!");
  })
